...
The genetic algorithm used is based off of the one developed by Steven L. and Brunton and J. Nathan Kutz of Cambridge. It is used to find the tuned gain of the responses for the PID controller. First, the known variables are declared, like the mass and volume of the ping pong ball, gravitational acceleration,and the density of air. Next, the coefficients of the transfer function are declared. C2 is the characteristic equation of the ball and pipe system. C3 is the coefficient that relates air speed to the pwm signal. These variables are then plugged into the transfer function: G=c3*c2/(s*(s+c2)). 
